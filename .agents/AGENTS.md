...
## 10. 블로그 대표 이미지 본문 내용 1:1 밀착 및 충실 원칙 (Blog Content-Fidelity Image Rule)
*   **3가지 블로그 타입 공통 적용**: 웹사이트 내 3가지 유형의 모든 블로그(BuildSelf 위인 습관, PetSelf 반려동물 케어, FreshSnap 식재료 보관) 대표 이미지(`heroImage`)를 생성하거나 교체할 때, 단순히 대상 피사체(식재료, 반려동물, 위인) 하나만 단독 컷으로 보여주는 데 그치지 않고, **반드시 해당 포스트 본문에 기술된 핵심 내용(핵심 습관 행위, 과학적 보관법 수칙, 필수 주의사항 등)에 100% 충실하고 본문 내용과 완벽히 부합하도록 정밀 연출**해야 합니다.
*   **본문 수칙과의 모순/충돌 절대 금지**: 본문 텍스트에서 "비닐 보관 금지", "개봉 캔 보관 금지", "냉장 보관 금지" 등 금지/주의 수칙으로 명시한 안 좋은 습관이나 도구가 대표 이미지에 잘못 표현되지 않도록, 프롬프트 작성 전 본문 텍스트 전체를 철저히 검토해야 합니다.
*   **핵심 팁의 시각적 직관화**: 
    *   **FreshSnap 식재료 보관**: 본문의 핵심 보관 비법(예: 감자 박스 속 싹 방지용 사과 1개 동봉, 대파 키친타월 스탠딩, 토르티야 사이 종이호일 끼우기, 두부 소금물 수납, 참치 캔 제거 후 유리 용기 수납 등)이 시각적으로 명확히 드러나 이미지 썸네일만 보고도 핵심 꿀팁을 직관적으로 알아볼 수 있어야 합니다.
    *   **BuildSelf 위인 습관**: 위인이 실제로 해당 습관을 수행하던 역사적·상황적 장면(예: 아인슈타인의 바이올린 연주, 데카르트의 침대 묵상 등)이 본문 텍스트와 1:1로 완벽히 부합해야 합니다.
    *   **PetSelf 반려동물 케어**: 해당 세부 품종 고유의 구체적인 케어 환경(예: 전용 수조, 습도 조절 사육장, 쿠션 등) 및 주의 행동이 본문 수칙과 정확히 일치해야 합니다.
