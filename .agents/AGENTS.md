...
## 7. BuildSelf 습관 블로그 원천 DB 분리 및 인메모리 컴파일 절대 원칙 (Habit Blog Dual-DB Separation & In-Memory Compilation Rule)
*   **원천 JSON 100% 순수 한국어(`ko`) Master 보존**: `apps/studio/src/data/blogs/habits/*.json` 원천 데이터 파일은 오직 100% 순수 한국어 Master(`ko`) 데이터만 영구히 깔끔하게 보관합니다. 원천 JSON 파일 자체에 타 외국어(`en`, `ja`, `zh`, `es`, `fr`, `de`, `pt`, `id`) 필드를 추가하거나 파일 내용 자체를 수정/덮어쓰는 행위를 **엄격히 금지**합니다.
*   **다국어 번역 사전 DB의 독립 분리**: 8개 외국어 정밀 직역 텍스트는 원천 JSON을 오염시키지 않도록 독립된 다국어 번역 DB(`apps/studio/src/data/blogs/habits_i18n/*.json`)에 슬러그(slug) 키 단위로 완벽히 분리 보관합니다.
*   **[2중 3중 절대 봉인] 번역 생성 스크립트 작성 및 실행 100% 영구 금지**:
    *   `replace()` 단어 치환 스크립트나 파라미터 템플릿 결합 스크립트 등 **번역 데이터를 자동으로 생성하려는 모든 형태의 스크립트 작성을 100% 영구 금지**합니다.
    *   모든 다국어 번역 DB(`habits_i18n/*.json`)는 **오직 AI 에이전트(LLM)가 한국어 원문을 직접 1:1로 읽고 단 한 문장도 축약 없이 100% 타겟 모국어로 완전 직역하여 파일 작성 도구로만 직접 생성**해야 합니다.
    *   이를 위반하고 번역 생성 스크립트를 작성하거나 가동하는 행위는 엄격한 정책 위반으로 간주합니다.
*   **[읽기 전용 검증 스크립트 전용 허용]**: 디스크 원본 데이터를 수정하지 않고 오직 유니코드 한글 잔류(`\uAC00-\uD7A3`) 및 타겟 언어 순도를 스캔하는 **읽기 전용(Read-Only) 검증 스크립트만 제한적으로 허용**합니다.
*   **1:1 본문 무생략 고품격 정밀 번역 보존**: 한국어 원문의 인물별 구체적인 역사적 에피소드, 뇌과학적 원리, 주의사항, 3단계 실천법, FAQ 본문 텍스트 전체를 단 한 문장도 축약/생략 없이 8개 외국어로 1:1 동일한 분량의 정밀 직역을 반영해야 합니다. (단, `Cognitive Neuroscience`, `Task-Switching Cost`, `Maya Angelou` 등 억지로 번역하면 이상해지는 학술 용어/인명 원어는 자연스럽게 보존)
*   **빌드 타임 인메모리(In-Memory) 결합 및 마크다운 직출력**: 컴파일 파이프라인(`compile_habit_blogs.js`)은 빌드 실행 시 '원천 한국어 DB'와 '독립 다국어 번역 DB'를 메모리 상에서만 조인(Join)하여, 원천 파일 손상 없이 최종 다국어 마크다운 파일(`apps/studio/src/content/blog/{lang}/*.md`) 2,664개로만 직접 출력해야 합니다.
