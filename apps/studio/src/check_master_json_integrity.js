...
jsonDirs.forEach(pattern => {
  ...
  files.forEach(filePath => {
    ...
    try {
      ...
      // 2. 핵심 텍스트 필드 9개 국어 슬롯 누락 및 언어 순도 검사
        ...
        requiredLangs.forEach(lang => {
          ...
          if (text === undefined || text === null || text === '') {
            if (['title', 'description'].includes(field)) {
              console.error(`\x1b[31m❌ [ERR] ${fileName}: '${field}.${lang}' 다국어 텍스트 슬롯이 누락되었습니다.\x1b[0m`);
              hasError = true;
            }
            return;
          }
          ...
          if (['en', 'es', 'fr', 'de', 'pt', 'id'].includes(lang)) {
            ...
            if (hasKorean || hasJapanese || hasChinese) {
              console.error(`\x1b[31m❌ [ERR] ${fileName} (${field}.${lang}): 라틴계열 원천 데이터에 이종 언어(한글/가나/한자)가 포함되어 있습니다.\x1b[0m`);
              hasError = true;
            }
          }
          ...
          if (lang === 'ko') {
            const hasJapanese = /[\u3040-\u309F\u30A0-\u30FF]/.test(text);
            if (hasJapanese) {
              console.error(`\x1b[31m❌ [ERR] ${fileName} (${field}.ko): 한국어 원천 데이터에 일본어(가나)가 포함되어 있습니다.\x1b[0m`);
              hasError = true;
            }
          }
          ...
          if (lang === 'ja') {
            const hasKorean = /[\uAC00-\uD7A3\u3130-\u318F]/.test(text);
            if (hasKorean) {
              console.error(`\x1b[31m❌ [ERR] ${fileName} (${field}.ja): 일본어 원천 데이터에 한국어(한글)가 포함되어 있습니다.\x1b[0m`);
              hasError = true;
            }
          }
          ...
          if (lang === 'zh') {
            ...
            if (hasKorean || hasJapanese) {
              console.error(`\x1b[31m❌ [ERR] ${fileName} (${field}.zh): 중국어 원천 데이터에 한국어/일본어가 유입되었습니다.\x1b[0m`);
              hasError = true;
            }
          }
        });
