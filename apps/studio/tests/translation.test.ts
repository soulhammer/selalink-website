    ...
    mdFiles.forEach((filePath) => {
      ...
      it(`[${lang.toUpperCase()}] ${fileName} 파일 정합성 검사`, () => {
        ...
        if (lang === 'ko') {
          const hasJapanese = /[\u3040-\u309F\u30A0-\u30FF]/g.test(sanitizedAll);
          const hasKorean = /[\uAC00-\uD7A3\u3130-\u318F]/g.test(sanitizedAll);
          const hasEnglishFallbackCta = /Smart storage & expiration alerts|Download on Google Play/i.test(sanitizedAll);
          ...
          expect(hasEnglishFallbackCta, `오류: 한국어 문서 ${fileName} 에 영어 fallback CTA 텍스트가 잘못 포함되어 있습니다.`).toBe(false);
        }
