...
describe('FreshSnap Astro UI 템플릿 유니코드 한글 누출 스캐너 테스트', () => {
  it('FreshSnap index.astro 템플릿 마크업 내 다국어 치환되지 않은 하드코딩 한글 UI 텍스트가 존재하지 않아야 한다', () => {
    ...
    if (leakedLines.length > 0) {
      console.error('⚠️ [FreshSnap HTML UI 누출 발견] index.astro 마크업 내 미번역 한글 하드코딩 라인들:', leakedLines);
    }
