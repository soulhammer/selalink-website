...
describe('BuildSnap 실제 위인 데이터 96종 매칭 정합성 검증', () => {
  it('실제 등록된 모든 위인(96명)이 최소 1개 이상의 진단 조합 결과에 노출되어야 한다 (누수 0명)', () => {
    ...
    // 전수 조사 결과 매칭 대상 목록에 모든 위인(196명)이 포함되어 있는지 확인
    expect(realHabits.length).toBe(196);
