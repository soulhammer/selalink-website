...
*   **본문 보조 이미지 미사용 정책**:
    *   본문 중간 보조 이미지는 복잡성을 방지하고 일관된 레이아웃을 제공하기 위해 **더 이상 사용하지 않으며, 오직 대표 이미지 1개만 사용합니다.** 따라서 마크다운 본문에 별도의 `<img>` 태그를 삽입하지 않습니다.
...
*   **구동 방식 (`npm run retrofit-blogs`)**:
    *   기존에 발행된 43개 전체 블로그를 전수 조사하여 다음 2대 결함을 진단(Lint)합니다:
        1.  **아코디언 FAQ 부재**: 본문 최하단 `<details>` 기반 아코디언 컴포넌트 유무.
        2.  **JSON-LD 구조화 태그 부재**: Frontmatter 내 `faqs` 메타데이터 누락 여부.
